...
def choose_best_split(data_set, leaf_type=reg_leaf, err_type=reg_err, ops=(1, 4)):
    ...
    for feat_dim in range(n - 1):
        for split_val in set(data_set[:, feat_dim].A.tolist()[0]):
            mat0, mat1 = bin_split_data(data_set, feat_dim, split_val)
    ...
    if (s - best_s) < tol_s:
        return None, leaf_type(data_set)
    mat0, mat1 = bin_split_data(data_set, best_dim, best_val)
    if np.shape(mat0)[0] < tol_n or np.shape(mat1)[0] < tol_n:
        return None, leaf_type(data_set)
